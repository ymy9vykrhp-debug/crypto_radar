...
10. **Research safety:** fixed train/validation/out-of-sample and guarded profile learning delivered. A cached paginated historical store and cost-aware execution simulator now cover configured fees, spread, slippage, partial exits and optional funding events. Full 6–12 month rolling walk-forward, verified historical microstructure/funding datasets, confidence intervals, Monte Carlo and what-if analysis remain.
...
Phase A also retains entry/stop variants, separate direction/location/entry/stop/liquidity/data/setup/risk quality scores, structural invalidation, dynamic stop buffer, false-breakout state, sweep/reclaim evidence, stop overshoot and `STOP_THEN_TARGET`. Fixed-profile reports include chronological train/validation/OOS slices with sample sizes. Cost-aware reports expose separate Raw/Net Average R, Profit Factor, Drawdown and execution costs. These slices remain diagnostic only; full rolling walk-forward and historically verified spread/funding inputs remain later research gates.
